...
Constraints
    ...
    C12  Utilisation target (soft): permanent/fixed_term employees should
         reach 40%/20% of their contract hours summed over the WHOLE
         horizon, not every single week — a light week can be made up later
...
def solve_allocation(employees, locations,
                     ...
                     utilisation_penalty=DEFAULT_UTILISATION_PENALTY,
                     stay_until_end=True,
    ...
    # C12. Horizon-level utilisation shortfall (soft target). For permanent
    # and fixed_term employees, short_i >= target_hours_i - total_assigned_i,
    # where target_hours_i is a fraction of (contract_hours * weeks active).
    # Penalised in the objective, NOT a hard constraint — a week with light
    # demand is fine as long as the employee catches up later in the horizon.
    UTIL_TARGET_FRACTION = {"permanent": 0.40, "fixed_term": 0.20}
    util_shortfall = {}
    util_target_hours = {}
    if utilisation_penalty > 0:
        for e in emp:
            ...
            util_shortfall[eid] = LpVariable(f"shortfall_{eid}", lowBound=0)
    ...
    utilisation_shortfall_penalty = lpSum(
        utilisation_penalty * util_shortfall[eid] for eid in util_shortfall
    )
    prob += (
        internal_cost + external_cost + switch_penalty
        + location_spread_penalty + utilisation_shortfall_penalty
    )
    ...
    # C12. Horizon-level utilisation target (soft). shortfall[eid] captures
    # how far total assigned hours over the horizon fall short of the
    # target (40% of contract_hours*weeks for permanent, 20% for
    # fixed_term). Being short in one week is fine as long as later weeks
    # make up for it — only the running total over the horizon matters.
    for eid, target in util_target_hours.items():
        pairs_emp = [(i, j, tt) for (i, j, tt) in eligible_pairs_t if i == eid]
        prob += (
            util_shortfall[eid] >= target - block_size * lpSum(k[p] for p in pairs_emp),
            f"util_shortfall_{eid}",
        )
    ...
    utilisation_shortfall_report = {
        eid: {
            "target_hours": util_target_hours[eid],
            "shortfall_hours": float(util_shortfall[eid].value() or 0),
        }
        for eid in util_shortfall
    }
    ...
    summary = {
        ...
        "utilisation_shortfall": utilisation_shortfall_report,
