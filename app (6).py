...
# ── Version-check banner (shows in sidebar so you can see which model is loaded)
# If C12 isn't hard or the fractions aren't 50/30, this banner will scream at you.
import inspect as _inspect
import model as _model_mod
_model_src = _inspect.getsource(_model_mod)
_c12_hard = "min_util_" in _model_src and "util_shortfall_" not in _model_src
_fractions_ok = '"permanent": 0.50' in _model_src and '"fixed_term": 0.30' in _model_src
_managers_excl = '"role") == "Manager"' in _model_src
_c9_present = "project_leader_" in _model_src

if _c12_hard and _fractions_ok and _managers_excl and _c9_present:
    st.sidebar.success("✅  Model: latest (hard C12 50/30, managers excluded, hard C9)")
else:
    st.sidebar.error(
        "❌  STALE model.py detected!\n\n"
        f"- C12 hard:        {'✅' if _c12_hard else '❌'}\n"
        f"- 50/30 fractions: {'✅' if _fractions_ok else '❌'}\n"
        f"- Managers excl.:  {'✅' if _managers_excl else '❌'}\n"
        f"- C9 present:      {'✅' if _c9_present else '❌'}\n\n"
        "Replace model.py with the latest version on GitHub, "
        "then click 'Reboot app' in Streamlit Cloud."
    )
