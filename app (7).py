...
fallback_rate = st.sidebar.number_input(
    "Fallback rate for employees with no rate in template (€/h)",
    value=22.0, step=1.0,
...
block_size = st.sidebar.selectbox(
    ...
    options=[0.5, 1, 2, 4],
    index=0,  # default = 0.5h, supports windows of any length divisible by 0.5h
    help="Internal assignments are integer multiples of this many hours. "
         "0.5 = half-hour granularity (supports non-integer windows like 5.5h). "
         "1 = integer hours only (faster but fails on non-integer windows).",
)
...
with tab_inputs:
    ...
    # Defensive: older data_loader.py versions may not produce 'cost_source'.
    # If the column is missing, assume every employee row came from the template.
        ...
        if assignments.empty:
            ...
            grouped = (
                ...
                .apply(lambda g: pd.Series({
                    "cell": " · ".join(
                        _label(r.get("project"), r["site"], r["hours"])
